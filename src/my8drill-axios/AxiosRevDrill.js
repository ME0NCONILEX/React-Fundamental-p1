...
const AxiosRevDrill = () => {
  ...
  const postRequestAction = async () => {
    ...
    const data = {firstName: 'new', lastName:'person', email: 'new.person@nonet.io', title: 'EXAMPLE'};
  ...
  const putRequestAction = async () => {
    ...
    const data = {id: 4, firstName: 'last', lastName:'persson', email: 'last.persson@nonet.io', title: 'TEST'};
    await axios.put(API_URL, data).then(response => {
      console.log("RESPONSE:", response);
      if(response.status === 204){
        setAlert({type: 'success', message: 'Ok, Put operation is done!'});
      } else {
        setAlert({type: 'warning', message: 'Display API Error Message...'});
      }
  ...
  return (
    <div className='container pt-5 bg-opacity-100'>
      <h3>AxiosRev Drill</h3>
      <ObsMessage message={alert.message} type={alert.type} />
      <div className='row'>
        <div className='col'>
